...
export default function Leaderboard() {
  return (
    <div className="bg-[#2A2B32] rounded-[10px] md:pt-2 py-2 px-4 md:pb-6 md:px-7 gap-2 h-full">
      <h5 className="text-center text-xl sm:text-2xl text-white mb-2">Leaderboard</h5>
      ...
      <div className="flex flex-col py-3 px-2 gap-[10px]">
        {leaderBoardData.map((item, index) => (
          <div
            ...
            key={`${item.name}-${index}`}
          >
            <span className="text-sm sm:text-lg text-black font-semibold">{item.id}</span>
